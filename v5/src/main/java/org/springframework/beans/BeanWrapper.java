 ...
 * thrown. Bulk updates continue after exceptions are encountered, throwing an exception wrapping <b>all</b> exceptions encountered during the update.
 *
...
public interface BeanWrapper {
     ...
     *
     * @param requiredType   type of the property, can be null if a property is given but should be specified in any case for consistency checking
     * @param propertyPath   path of the property (name or nested path), or null if registering an editor for all properties of the given type
     * @param propertyEditor editor to register
     */
     ...
     *
     * @param requiredType type of the property, can be null if a property is given but should be specified in any case for consistency checking
     * @param propertyPath path of the property (name or nested path), or null if looking for an editor for all properties of the given type
     ...
     * @throws FatalBeanException if there is no such property, if the property isn't readable, or if the property getter throws an exception.
     */
     ...
     * provided for convenience. Behaviour will be identical to that of the setPropertyValues(PropertyValues) method.
     *
     * @param map Map to take properties from. Contains property value objects, keyed by property name
     */
     ...
     * in that an implementation of this class will continue to update properties if a <b>recoverable</b> error (such as a vetoed property change or a type mismatch, but <b>not</b> an invalid fieldname or the like) is encountered, throwing a PropertyAccessExceptionsException containing all the
     * individual errors. This exception can be examined later to see all binding errors. Properties that were successfully updated stay changed.
    ...
    /**
     * Perform a bulk update with full control over behavior. Note that performing a bulk update differs from performing a single update, in that an implementation of this class will continue to update properties if a <b>recoverable</b> error (such as a vetoed property change or a type mismatch, but
     * <b>not</b> an invalid fieldname or the like) is encountered, throwing a PropertyAccessExceptionsException containing all the individual errors. This exception can be examined later to see all binding errors. Properties that were successfully updated stay changed.
     * <p>Does not allow unknown fields.
     *
     * @param pvs           PropertyValues to set on the target object
