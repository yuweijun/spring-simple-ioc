...
/**
 * Extension of the BeanFactory interface to be implemented by bean factories that can enumerate all their bean instances, rather than attempting bean lookup by name one by one as requested by clients. BeanFactory implementations that preload all their beans (for example, DOM-based XML factories)
 * may implement this interface. This interface is discussed in "Expert One-on-One J2EE Design and Development", by Rod Johnson.
 *
 * <p>If this is a HierarchicalBeanFactory, the return values will <i>not</i> take
 * any BeanFactory hierarchy into account, but will relate only to the beans defined in the current factory. Use the BeanFactoryUtils helper class to get all beans.
 *
 * <p>The methods in this interface will just respect bean definitions of this factory.
 * They will ignore any singleton beans that have been registered by other means like ConfigurableBeanFactory's registerSingleton. Use BeanFactory's methods to access such beans. In typical scenarios, all beans will be defined by bean definitions anyway.
 *
...
public interface ListableBeanFactory extends BeanFactory {
    ...
    /**
     * Return the names of beans matching the given type (including subclasses), judging from the bean definitions. Will <i>not</i> consider FactoryBeans, as the type of their created objects is not known before instantiation. Does not consider any hierarchy this factory may participate in.
     * <p>Note: Ignores any singleton beans that have been registered by
     * other means than bean definitions.
     *
     * @param type class or interface to match, or null for all bean names
     * @return the names of beans matching the given object type (including subclasses), or an empty array if none
     */
     ...
     * singleton, the type of the actually created objects should be evaluated. Prototypes without explicit object type specification should be ignored.
     * <p>Note: Does <i>not</i> ignore singleton beans that have been registered
     * by other means than bean definitions.
     *
     * @param type                class or interface to match
     * @param includePrototypes   whether to include prototype beans too or just singletons (also applies to FactoryBeans)
